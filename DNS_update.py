        ...
        #If EC2 is terminated, check if its instance_id on DB table, if match, remove it from DB and remove A record on Route 53 private zone, else don't do it and alarm.
            ...
            if (items['Count'] == 0):
                ...
                if (instance_id == instance_id_store_on_DB):
                    ...
                    LOGGER.debug("This hostname is available but terminated EC2 instance id is not match, instance_id stored in DB is %s, terminated EC2 instance id is %s", instance_id_store_on_DB, instance_id)
    else: 
        LOGGER.debug("There is not any Name in Tags")